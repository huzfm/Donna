...
const AGENT_TOOLS: Groq.Chat.ChatCompletionTool[] = [
  {
    ...
    function: {
      ...
      description: "Semantic search over the user's uploaded files. Use for any document question.",
      parameters: {
        type: "object",
        properties: {
  ...
  },
  {
    ...
    function: {
      ...
      description: "Fetch ALL uploaded document chunks in order. Use for diagrams or full-file summaries.",
  ...
  for (let iteration = 0; iteration < maxIterations; iteration++) {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        // Model failed to produce a valid tool call — retry once without tools
        const isToolUseFailed =
          errMsg.includes("tool_use_failed") || errMsg.includes("Failed to call a function");
        if (isToolUseFailed) {
