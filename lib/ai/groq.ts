...
/** Recomputed on every call so the date never goes stale on a long-running server. */
...
function getSystemPrompt(): string {
  return `You are Donna, a personal AI workspace assistant. Today: ${getToday()}.
...
- Diagrams or full-file context → call get_all_documents, then optionally provide a Mermaid diagram.
...
- Use --> arrows.
- Use unique node IDs.
- Do not use parentheses in labels.
- Never use "end" or "start" as bare node IDs.
- When diagrams are requested, provide valid Mermaid syntax.`;
}

/** Backward-compatible export for anything importing the old constant directly. */
...
/** Route to a stronger model for complex/long tasks, a faster model otherwise. */
function pickModel(messages: AgentMessage[]): string {
  ...
  const isComplex =
    text.length > 150 ||
    /summarize|analyz|compar|explain|generat|diagram|list all|overview|write|draft/i.test(
      text
    );

  return isComplex ? STRONG_MODEL : FAST_MODEL;
}
...
export async function askGroq(
  ...
  const systemContent = opts?.systemPrompt ?? getSystemPrompt();
