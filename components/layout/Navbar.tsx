...
export default function Navbar() {
  ...
  return (
    <>
      <motion.div
        className="bg-accent fixed top-0 right-0 left-0 z-[60] h-[3px] origin-left"
        style={{ scaleX }}
      />
      <motion.header
        ...
        className={`fixed top-0 right-0 left-0 z-50 transition-all duration-300 ${
          scrolled
            ? "border-border border-b bg-white/80 shadow-sm backdrop-blur-lg"
            : "bg-transparent"
        }`}
      >
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          <Link href="/" className="flex items-center gap-2">
            ...
            <span className="text-primary text-lg font-semibold tracking-tight">Donna</span>
          </Link>
